...
1. Upload a file (Tucker Davis .arf, EPFL .tsv or .asc, or standardized .csv). If you're uploading a .csv, there must be a `Level(dB)` column and a `Freq(Hz)` column, and the vector of data points ends each corresponding row.
   * dB saved as attenuation (.arf only): Enable this option if ABR stimulus levels are stored as attenuation values (relative to the calibrated maximum output) rather than absolute dB SPL, as is the case for some .arf files generated by BiosigRP-based acquisition systems.
